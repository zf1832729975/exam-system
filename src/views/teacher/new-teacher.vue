<template>
    <div>
        <div class="meta">
            专业:
            <el-select v-model="newStudentClassId" placeholder="请选择专业">
              ...
              <table class="table">
                ...
                <tbody>
                    <tr v-for="i in tableRowLength" :key="i">
                            <td>{{i}}</td>
                            <td><input type="text" v-model="data[i - 1][0]"></input></td>
                            <td><input type="text" v-model="data[i - 1][1]"></input></td>
                            <td>
                                <el-switch
                                    v-model="data[i - 1][2]"
                                    active-color="#539fed"
                            ...
                            <td><input type="text" v-model="data[i - 1][3]"></input></td>
                    </tr>
                </tbody>
    ...
    methods: {
		...
		initNewStudentList () {
			for (let i = 0; i < this.tableRowLength; i++) {
				if (!this.data[i])
					this.data.push(['', '', this.defaultSex, this.defaultPassword])
			}
			console.log('this.data', this.data)
		},
		// 清空新增学生数据的姓名
		clearNewStudentList () {
            this.data = []  // 解决内容更新视图没有更新
			for (let i = 0; i < this.tableRowLength; i++) {
				this.data[i] = ['', '', this.defaultSex, this.defaultPassword]
			}
		},
        // 保存学生
		submit () {
			...
			for (let i = 0; i < len; i ++) {
				...
				} else if (this.data[i][0].length === 1) {
