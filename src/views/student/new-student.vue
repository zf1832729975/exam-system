<template>
	<div class="add-student">
		<div class="meta">
			...
			<span>说明：学号不输入自动长，姓名为字符少于等于1将被忽略, 密码为空设置默认密码123456</span>
		</div>
		<div class="table">
			<table class="add-student-table">
				...
				<tbody>
					<tr v-for="i in tableRowLength" :key="i">
						<td>{{i}}</td>
						<td>
							<input type="text" v-model="newStudentList[i - 1][0]">
						</td>
						<td>
							<input type="text" v-model="newStudentList[i - 1][1]">
						</td>
						<td>
							<el-switch
								v-model="newStudentList[i - 1][2]"
								active-color="#539fed"
						...
						<td>
							<input type="text" v-model="newStudentList[i - 1][3]">
						</td>
					</tr>
				</tbody>
...
export default {
	...
	methods: {
			...
			for (let i = 0; i < this.tableRowLength; i++) {
				if (!this.newStudentList[i])
					this.newStudentList.push([
						null,
						"",
						this.newStudentDefaultSex,
						this.newStudentDefaultPassword
					]);
			}
		...
		saveStudent() {
            ...
            // (id, name, sex, password, classId)
			for (let i = 0; i < len; i++) {
				if (this.newStudentList[i][1].length > 1) {
					//姓名不能小于两个个字
					if (this.newStudentList[i][3].length >= 6) {
						// 密码不小于 6 位
						this.newStudentList[i][4] = classId;
						data.push(this.newStudentList[i]);
					} else {
						this.$message(`第 ${i + 1} 行的密码不能少于6字个符`);
					}
				} else if (this.newStudentList[i][1].length === 1) {
